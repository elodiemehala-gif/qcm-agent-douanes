...
for(let idx=0;idx<raw.length;idx++){
 const r=raw[idx],xs=grouped.get(r.id)||[],c=byCat[r.cat]||(byCat[r.cat]={facts:0,pass:0,noQuestion:0,flagged:0,sourceRejected:0});c.facts++;
 if(!xs.length){
   ...
   rows.push({id:r.id,cat:r.cat,page:r.page,chapter:null,topic:null,source:r.text,allPass:false,anyQuestion:false,issues,sourceRejected:true,variants:[]});
   continue;
 }
 const variants=[];let rawAllPass=true,rawAnyQuestion=false;const rawIssues=[];
 for(let xi=0;xi<xs.length;xi++){
   ...
   rawAllPass=rawAllPass&&allPass;rawAnyQuestion=rawAnyQuestion||anyQuestion;rawIssues.push(...union);
 ...
 const union=[...new Set(rawIssues)];for(const z of union)issueCounts[z]=(issueCounts[z]||0)+1;
 if(rawAllPass)c.pass++;else c.flagged++;if(!rawAnyQuestion)c.noQuestion++;
 rows.push({id:r.id,cat:r.cat,page:r.page,chapter:variants[0]?.chapter||null,topic:variants[0]?.topic||null,source:r.text,allPass:rawAllPass,anyQuestion:rawAnyQuestion,issues:union,sourceRejected:false,variants});
}
const summary={generatedAt:new Date().toISOString(),rawFacts:raw.length,enrichedFacts:K.length,auditedSourceFacts:rows.length,categories:cats,byCat,issueCounts,fullyCompliant:rows.filter(r=>r.allPass).length,flagged:rows.filter(r=>!r.allPass).length,noQuestion:rows.filter(r=>!r.anyQuestion).length,withQuestion:rows.filter(r=>r.anyQuestion).length,sourceRejected:rows.filter(r=>r.sourceRejected).length};
