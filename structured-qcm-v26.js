(()=>{
 ...
 function prompt(z){const f=norm(z.field),s=z.subject;
   ...
   if(/^date(?: de début)?$/.test(f)||/événement/.test(f)&&type(z.answer)==='year')return `Quelle date est associée à « ${s} » ?`;
   ...
   if(/rôle|mission|fonction/.test(f))return `Quel est le rôle principal de ${s} ?`;
   if(/symbole/.test(f))return `Quel symbole est associé à ${s} ?`;
   if(/domaine/.test(f))return `Dans quel domaine classe-t-on ${s} ?`;
   if(/définition|sens|concept|principe/.test(f))return `Que désigne « ${s} » ?`;
   ...
   if(/type \/ repère|^repère$/.test(f))return `Quel repère est associé à ${s} ?`;
   if(/repère stratégique/.test(f))return `Quel repère stratégique concerne ${s} ?`;
   if(/caractéristiques?|statut|lien/.test(f))return `Quelle caractéristique définit ${s} ?`;
